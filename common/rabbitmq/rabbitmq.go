...
func (r *RabbitMQ) PublishSimple(message string) error {
	...
	r.channel.Publish(
		r.Exchange,
		r.QueueName,
		false, // 如果为true，根据exchange类型和 routkey 规则，如果无法找到符合条件的队列那么会吧发送的消息回退给发送者
...
func (r *RabbitMQ) ConsumeSecondServe(orderService service.IOrderService, productService service.IProductService) {
	// 1.申请队列，如果队列不存在会自动创建，如果存在则跳过创建。保证队列存在消息能发送到队列中
...
func (r *RabbitMQ) ConsumeCache(product dao.IProduct) {
	// 1.申请队列，如果队列不存在会自动创建，如果存在则跳过创建。保证队列存在消息能发送到队列中
	q, err := r.channel.QueueDeclare(
		r.QueueName,
		false, // 是否持久化
	...
	go func() {
		for d := range msgs {
			...
			// 添加商品
			if !message.IsDelete {
				_, err = product.InsertCache(message)
				if err != nil {
					fmt.Println(err)
				}
			} else {
				ok, err := product.DeleteCache(message.ID)
				if !ok {
					fmt.Println(err)
				}
			}
